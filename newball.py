...
L2 = sqrt((balls[0].obj.pos.x - balls[1].obj.pos.x)**2 + (balls[0].obj.pos.y - balls[1].obj.pos.y)**2 + (balls[0].obj.pos.z - balls[1].obj.pos.z)**2)
...
center_ball = sphere(pos=avgpos, color=color.green, radius=0.2)
...
while True:
    ...
    for i in range(len(balls) - 1):
        ...
        print(mag(connect_spring[i].obj.axis), L2)
    ...
    for i in range(0, len(balls)):
        ...
        forces[i] = (getforce(center_spring[i].obj.axis, connect_spring[i - 1].obj.axis, connect_spring[i].obj.axis) \
                     + SpringDamp(balls[i].obj.v, balls[i].obj.pos) + springForce(balls[i].obj.pos - balls[i - int(ballnum / 2)].obj.pos, 2 * R))
        #arrow(pos=balls[i].obj.pos, axis=forces[i], color=color.white, shaftwidth=0.1)
        # print(mag(balls[i].obj.pos - top) - R)
        # print(mag(center_spring[i].obj.axis) - L1)
        # print()
        # for j in range(5):
        #     forcearr[i][j].pos = balls[i].obj.pos

        #forcearr[i][0].axis = 2 * norm(springForce(center_spring[i].obj.axis, L1) - springForce(balls[i - int(ballnum / 2)].obj.pos - balls[i].obj.pos, 2 * R))
        #forcearr[i][0].axis = norm(springForce(balls[i].obj.pos - balls[i - int(ballnum / 2)].obj.pos, 2 * R))
        # forcearr[i][1].axis = 2 * norm(- springForce(connect_spring[i - 1].obj.axis, L2))
        # forcearr[i][2].axis = 2 * norm(springForce(connect_spring[i].obj.axis, L2))
        # forcearr[i][3].axis = 2 * norm(Fg)
        # print(mag(connect_spring[i - 1].obj.axis) - L2)
        #print(forcearr[i].axis)
        #print(springForce(balls[i].obj.pos - top, R))
        #print(forces[i], end=" ")
    ...
    for i in range(len(balls)):
        ...
        forcearr[i][4].axis = norm(balls[i].obj.v)
